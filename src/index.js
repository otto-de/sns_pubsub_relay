...
const expectedTopicArn = process.env.SNS_TOPIC;
  ...
  validator.validate(JSON.parse(req.body), async function (err, message) {
    ...
    switch (message.Type.toLowerCase()) {
      ...
      case 'notification':
        ...
        let msgData = Buffer.from(message.Message);
        const messageId = await pubSubClient.topic(pubsubTopic).publish(msgData, attributes);
